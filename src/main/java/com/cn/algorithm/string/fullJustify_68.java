    ...
    **/
    ...
    public static List<String> fullJustify(String[] words, int maxWidth) {
        ...
        while (index < len){
            String temp = "";
            int length = 0;
            while (index < len && length < maxWidth / 2){
                temp += words[index] + ",";
            ...
            int block = maxWidth - cur;
            int[] blocks = new int[num - 1];
            blocks[0] = block / (num - 1);
            for(int i = 1; i < num - 1; i++){
                blocks[i] = (block - blocks[i - 1]) / (num - i - 1);
            }
            String anss = "";
            for(int k = 0;k < num; k++) {
                anss += split[k];
                for (int i = 0; k < num - 1 && i < blocks[k]; i++) {
                    anss += " ";
                }
            }
            ans.add(anss);
    ...
    public static List<String> fullJustify_right(String[] words, int maxWidth) {
